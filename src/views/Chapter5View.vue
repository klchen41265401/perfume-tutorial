<template>
  <div>
    <h1>第5章：配方工程</h1>
    <ChapterNav :prev="{ to: '/chapter/4', label: '第4章：合成' }" :next="{ to: '/chapter/6', label: '第6章：分析方法' }" />
...
C(x,t) = C₀·exp(-√(k/D)·x)·[1 - erf(x/√(4Dt))]</code></pre>
    ...
    <h3>5.1.2 多組分競爭揮發</h3>
    <div class="chart-container"><canvas ref="evaporationChart" style="max-height: 400px;"></canvas></div>
...
約束：∑xᵢ = 1, xᵢ ≥ 0, safety_indexᵢ ≤ threshold</code></pre>
    <div class="chart-container"><canvas ref="paretoChart" style="max-height: 400px;"></canvas></div>
...
→ 輸出 Pareto 前沿解集</code></pre>
    ...
    <div class="chart-container"><canvas ref="customFormulaChart" style="max-height: 400px;"></canvas></div>
...
大眾品牌: VI > 2.0</code></pre>
    <div class="chart-container"><canvas ref="valueChart" style="max-height: 400px;"></canvas></div>
...
export default {
  ...
  setup() {
    ...
    const molecules = [
      { name: '檸檬烯 (Limonene)', smiles: 'CC1=CCC(CC1)C(=C)C' },
      { name: '芳樟醇 (Linalool)', smiles: 'CC(=CCCC(C)(O)C=C)C' },
      { name: '香豆素 (Coumarin)', smiles: 'O=C1OC2=CC=CC=C2C=C1' },
      { name: '酮麝香', smiles: 'CC1=CC([N+](=O)[O-])=C(C)C([N+](=O)[O-])=C1' },
      { name: '玫瑰氧化物', smiles: 'CC1CCC(OC1)C(C)=C' }
    ]
    ...
    return { evaporationChart, paretoChart, customFormulaChart, valueChart, citrus, linalool, rose, sandalwood, musk, totalPct, updateFormulation, molecules }
  }
}
</script>
