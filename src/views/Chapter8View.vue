<template>
  <div>
    <h1>第8章：進階主題與未來趨勢</h1>
    <ChapterNav :prev="{ to: '/chapter/7', label: '第7章：案例研究' }" :next="null" />
...
訓練：EarlyStopping(patience=15), ReduceLROnPlateau</code></pre>
    <div class="chart-container"><canvas ref="mlPredictionChart" style="max-height: 400px;"></canvas></div>
...
應用：在潛在空間中插值生成新分子</code></pre>
    ...
    <div class="chart-container"><canvas ref="microfluidicsChart" style="max-height: 400px;"></canvas></div>
...
D = 1×10⁻¹² m²/s (Linalool)</code></pre>
    <div class="chart-container"><canvas ref="releaseChart" style="max-height: 400px;"></canvas></div>

    <hr>
    <h2>8.4 納米材料在香料中的應用</h2>
...
性能指標：準確率、召回率、F1-Score、覆蓋率、多樣性、新穎性</code></pre>
    <div class="chart-container"><canvas ref="recommendationChart" style="max-height: 400px;"></canvas></div>
    ...
    <p style="text-align: center; font-size: 1.4em; margin-top: 3rem;">
...
export default {
  ...
  setup() {
    ...
    const molecules = [
      ...
      { name: '樟腦 (Camphor)', smiles: 'CC1(C)C2CCC1(C)C(=O)C2' },
      { name: '香豆素 (Coumarin)', smiles: 'O=C1OC2=CC=CC=C2C=C1' },
      { name: '牻牛兒酸', smiles: 'CC(=CCCC(C)=CC(=O)O)C' },
      { name: '檸檬烯', smiles: 'CC1=CCC(CC1)C(=C)C' }
    ]
    ...
    return { mlPredictionChart, microfluidicsChart, releaseChart, recommendationChart, molecules }
  }
}
</script>
